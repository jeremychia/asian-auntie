...
def upgrade():
    ...
    with op.batch_alter_table('item_photos', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_item_photos_item_id'), ['item_id'], unique=False)

    # Migrate existing single photo_path values into item_photos
    conn = op.get_bind()
    rows = conn.execute(sa.text("SELECT id, photo_path FROM items WHERE photo_path IS NOT NULL"))
    for row in rows:
        conn.execute(
            sa.text(
...
def downgrade():
    ...
    for row in rows:
        conn.execute(
            sa.text("UPDATE items SET photo_path = :path WHERE id = :id AND photo_path IS NULL"),
            {"path": row[1], "id": row[0]},
        )
